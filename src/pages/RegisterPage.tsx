    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-ink-950 px-4 py-16">
      ...
      <div className="relative w-full max-w-md animate-scale-in">
        ...
        <div className="border border-cream-200/15 bg-ink-900/60 p-8 backdrop-blur-xl">
          <h1 className="text-center font-display text-3xl font-bold text-cream-50">Create Account</h1>
          <p className="mt-2 text-center font-serif text-base text-cream-200/60">Join the ADS inner circle</p>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-[0.15em] text-cream-200/70">Full Name</span>
              <div className="mt-2 flex items-center gap-3 border-b border-cream-200/20 pb-2 focus-within:border-gold-400">
                <User size={16} className="text-cream-200/40" />
                <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="flex-1 bg-transparent text-sm text-cream-50 placeholder:text-cream-200/30 focus:outline-none" placeholder="Your name" />
              </div>
            </label>
            <label className="block">
            ...
            <label className="block">
              ...
              <div className="mt-2 flex items-center gap-3 border-b border-cream-200/20 pb-2 focus-within:border-gold-400">
                ...
                <input type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} className="flex-1 bg-transparent text-sm text-cream-50 placeholder:text-cream-200/30 focus:outline-none" placeholder="Min. 6 characters" />
              </div>
            </label>
          ...
          <p className="mt-6 text-center text-sm text-cream-200/60">
            Already a member?{' '}
            <Link to="/login" className="font-semibold text-gold-400 hover:text-gold-300">Sign in</Link>
          </p>
        </div>
